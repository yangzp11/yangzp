...
public class RedisUtil {
     ...
     * @return true成功 false 失败
     */
    public Boolean set(String key, Object value, Integer time, TimeUnit minutes) {
        try {
            if (time > 0) {
     ...
     * @param member       排行榜对象 +1 因为排名 0 开始
     * @return
     */
    public Long getReverseRankNum(String sortedSetKey, String member) {
        try {
            return redisTemplate.opsForZSet().reverseRank(sortedSetKey, member) + 1;
        } catch (Exception e) {
            e.printStackTrace();
            return 0l;
        }
     ...
     * @return Boolean
     */
    public Boolean listStringSet(String key, List<String> value) {
        try {
            redisTemplate.opsForList().rightPushAll(key, value);
            return true;
        } catch (Exception e) {
